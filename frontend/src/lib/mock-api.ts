// Mock API functions that simulate backend responses with real dataset
...
const STORAGE_KEY = "userDefinedTariffs"

const loadUserDefinedTariffs = (): Array<{
  id: string
  product: string
  exportingFrom: string
  importingTo: string
...
const saveUserDefinedTariffs = (tariffs: any[]) => {
  if (typeof window === "undefined") return
  try {
...
const userDefinedTariffs: Array<{
  id: string
  product: string
  exportingFrom: string
  importingTo: string
  ...
  effectiveDate: string
  expirationDate: string
}> = loadUserDefinedTariffs()

export const getUserDefinedTariffs = () => {
  return userDefinedTariffs
}

export const addUserDefinedTariff = (tariff: any) => {
  userDefinedTariffs.push(tariff)
  saveUserDefinedTariffs(userDefinedTariffs)
}

export const getProductsForSource = (source: string): string[] => {
  if (source === "user") {
    const products = new Set<string>()
    userDefinedTariffs.forEach((tariff) => {
      products.add(tariff.product)
    })
...
export const getCountriesForSource = (source: string): string[] => {
  if (source === "user") {
    const countries = new Set<string>()
    userDefinedTariffs.forEach((tariff) => {
      countries.add(tariff.exportingFrom)
      countries.add(tariff.importingTo)
    })
    return Array.from(countries).sort()
  }
  return getCountries()
...
export const getBrandsForProductAndSource = (productName: string, source: string): Product[] => {
  if (source === "user") {
    // For user-defined tariffs, we still use the global product brands
    // but only if the product exists in user-defined tariffs
    const userProducts = getProductsForSource("user")
    if (userProducts.includes(productName)) {
      return getBrandsForProduct(productName)
    }
    return []
  }
  return getBrandsForProduct(productName)
...
export const mockCalculateTariff = async (formData: any): Promise<TariffCalculationResponse> => {
  ...
  if (formData.tariffSource === "user") {
    // Look for user-defined tariff
    const userTariff = userDefinedTariffs.find(
      (t) =>
        t.product === formData.product &&
        t.exportingFrom === formData.exportingFrom &&
        t.importingTo === formData.importingTo,
    )

    if (!userTariff) {
      return {
        success: false,
        error: "No user-defined tariff found for this product and country combination",
      }
    }

    tariffRate = userTariff.rate
    tariffType = `${userTariff.type} (User Defined)`
  } else {
    // Use global tariff data
    tariffData = TARIFF_RATES.find((t) => t.country === formData.importingTo && t.partner === formData.exportingFrom)
    ...
    tariffRate = hasFTAStatus ? tariffData.ahsWeighted : tariffData.mfnWeighted
  ...
  return {
    ...
    data: {
      ...
      breakdown: [
        ...
        {
          description: `Import Tariff (${tariffType})`,
          type: "Tariff",
          rate: `${tariffRate.toFixed(2)}%`,
          amount: tariffAmount,
        },
      ],
    },
